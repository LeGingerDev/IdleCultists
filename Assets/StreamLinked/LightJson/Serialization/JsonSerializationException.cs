...
namespace ScoredProductions.StreamLinked.LightJson.Serialization
{
	...
	public sealed class JsonSerializationException : Exception
	{
		...
		/// Initializes a new instance of JsonSerializationException.
		/// </summary>
		public JsonSerializationException(Exception exc) : base(GetDefaultMessage(ErrorType.Unknown), exc) { }
		...
		public JsonSerializationException(ErrorType type) : base(GetDefaultMessage(type)) {
			this.Type = type;
		}
		...
		/// </summary>
		/// <param name="type">The error type that describes the cause of the error.</param>
		public JsonSerializationException(ErrorType type, Exception exc) : base(GetDefaultMessage(type), exc) {
			this.Type = type;
		}
		...
		/// <param name="type">The error type that describes the cause of the error.</param>
		public JsonSerializationException(string message, ErrorType type) : base(GetDefaultMessage(type, message)) {
			this.Type = type;
		}
		...
		/// <param name="message">The message that describes the error.</param>
		/// <param name="type">The error type that describes the cause of the error.</param>
		public JsonSerializationException(string message, ErrorType type, Exception exc) : base(GetDefaultMessage(type, message), exc) {
			this.Type = type;
		}
		...
		private static string GetDefaultMessage(ErrorType type, string message = null)
		{
			if (string.IsNullOrWhiteSpace(message)) {
				...
				return type switch {
					ErrorType.InvalidNumber => "The value been serialized contains an invalid number value (NAN, infinity). Provided Message: ",
					ErrorType.InvalidValueType => "The value been serialized contains (or is) an invalid JSON type. Provided Message: ",
					ErrorType.CircularReference => "The value been serialized contains circular references. Provided Message: ",
					_ => "An error occurred during serialization. Provided Message: ",
				} + message;
			}
		}
