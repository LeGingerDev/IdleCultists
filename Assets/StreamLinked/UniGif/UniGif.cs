    ...
    public static partial class UniGif {
        ...
        public static IEnumerator GetTextureListCoroutine(
            ...
            if (SetGifData(bytes, ref gifData, debugLog) == false) {
				DebugManager.LogMessage("GIF file data set error.", DebugManager.ErrorLevel.Error);
                if (callback != null) {
                    callback(null, loopCount, width, height);
				}
				List<GifTexture> l = null;
				yield return (l, loopCount, width, height);
				yield break;
            }
            ...
            if (gifTexList == null || gifTexList.Count <= 0) {
                DebugManager.LogMessage("GIF texture decode error.", DebugManager.ErrorLevel.Error);
                if (callback != null) {
                    callback(null, loopCount, width, height);
                }
                List<GifTexture> l = null;
            ...
            yield return (gifTexList, loopCount, width, height);
        }
		...
		/// <param name="wrapMode">Textures wrap mode</param>
		/// <param name="debugLog">Debug Log Flag</param>
		/// <returns>(List<GifTexture>, int, int, int)</returns>
		public static (List<GifTexture>, int, int, int) GetTextureList(byte[] bytes, FilterMode filterMode = FilterMode.Bilinear, TextureWrapMode wrapMode = TextureWrapMode.Clamp, bool debugLog = false) {
			int loopCount = -1;
			int width = 0;
			int height = 0;
			...
			if (SetGifData(bytes, ref gifData, debugLog) == false) {
				DebugManager.LogMessage("GIF file data set error.", DebugManager.ErrorLevel.Error);
                return (null, loopCount, width, height);
			}
            ...
            return (gifTexList, loopCount, width, height);
		}
		...
		/// <param name="debugLog">Debug Log Flag</param>
		/// <returns>(List<GifTexture>, int, int, int)</returns>
		public static (List<GifTexture>, int, int, int) GetTextureList(NativeArray<byte> bytes, FilterMode filterMode = FilterMode.Bilinear, TextureWrapMode wrapMode = TextureWrapMode.Clamp, bool debugLog = false) {
			int loopCount = -1;
			int width = 0;
			int height = 0;
