...
namespace ScoredProductions.StreamLinked.UniGif {
    public static partial class UniGif {
		...
		/// <param name="filterMode">Textures filter mode</param>
		/// <param name="wrapMode">Textures wrap mode</param>
		/// <returns>List<GifTexture></returns>
            ...
            for (int x = 0; x < tex.width; x++) {
                ...
                if (dataIndex >= decodedData.Length) {
                    if (filledTexture == false) {
                        tex.SetPixel(x, y, bgColor);
                        if (dataIndex == decodedData.Length) {
							DebugManager.LogMessage("dataIndex exceeded the size of decodedData. dataIndex:" + dataIndex + " decodedData.Length:" + decodedData.Length + " y:" + y + " x:" + x, DebugManager.ErrorLevel.Error);
                        }
                    }
                    dataIndex++;
                    continue;
                ...
                // Get pixel color from color table
                {
                    ...
                    if (colorTable == null || colorTable.Count <= colorIndex) {
                        if (filledTexture == false) {
                            tex.SetPixel(x, y, bgColor);
                            if (colorTable == null) {
								DebugManager.LogMessage("colorIndex exceeded the size of colorTable. colorTable is null. colorIndex:" + colorIndex, DebugManager.ErrorLevel.Error);
                            }
                            else {
								DebugManager.LogMessage("colorIndex exceeded the size of colorTable. colorTable.Count:" + colorTable.Count + " colorIndex:" + colorIndex, DebugManager.ErrorLevel.Error);
                            }
                        }
                        dataIndex++;
                        continue;
        ...
        private static byte[] DecodeGifLZW(List<byte> compData, int lzwMinimumCodeSize, int needDataSize) {
            ...
            while (bitDataIndex < bitData.Length) {
                ...
                if (key == clearCode) {
					...
					// Exit
					DebugManager.LogMessage("early stop code. bitDataIndex:" + bitDataIndex + " lzwCodeSize:" + lzwCodeSize + " key:" + key + " dic.Count:" + dic.Count, DebugManager.ErrorLevel.Warning);
                    break;
                }
                    ...
                    if (prevEntry != null) {
						...
						DebugManager.LogMessage("It is strange that come here. bitDataIndex:" + bitDataIndex + " lzwCodeSize:" + lzwCodeSize + " key:" + key + " dic.Count:" + dic.Count, DebugManager.ErrorLevel.Warning);
                        bitDataIndex += lzwCodeSize;
                        continue;
                    }
                }
