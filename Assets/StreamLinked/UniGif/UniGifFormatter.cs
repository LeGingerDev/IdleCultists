...
namespace ScoredProductions.StreamLinked.UniGif {
    public static partial class UniGif {
        ...
        private static bool SetGifData(byte[] gifBytes, ref GifData gifData, bool debugLog) {
            if (debugLog) {
				DebugManager.LogMessage("SetGifData Start.");
            }

            if (gifBytes == null || gifBytes.Length <= 0) {
				DebugManager.LogMessage("bytes is nothing.", DebugManager.ErrorLevel.Error);
                return false;
            }
            ...
            if (SetGifHeader(gifBytes, ref byteIndex, ref gifData) == false) {
                DebugManager.LogMessage("GIF header set error.", DebugManager.ErrorLevel.Error);
                return false;
            }

            if (SetGifBlock(gifBytes, ref byteIndex, ref gifData) == false) {
                DebugManager.LogMessage("GIF block set error.", DebugManager.ErrorLevel.Error);
                return false;
            }

            if (debugLog) {
                gifData.Dump();
				DebugManager.LogMessage("SetGifData Finish.");
            }
            return true;
        }
        ...
        /// <param name="debugLog">Debug log flag</param>
        /// <returns>Result</returns>
        private static bool SetGifData(NativeArray<byte> gifBytes, ref GifData gifData, bool debugLog) {
            if (debugLog) {
				DebugManager.LogMessage("SetGifData Start.");
            }
        ...
        private static bool SetGifHeader(byte[] gifBytes, ref int byteIndex, ref GifData gifData) {
            ...
            if (gifBytes[0] != 'G' || gifBytes[1] != 'I' || gifBytes[2] != 'F') {
                DebugManager.LogMessage("This is not GIF image.");
                return false;
            }
            ...
            if ((gifBytes[3] != '8' || gifBytes[4] != '7' || gifBytes[5] != 'a') &&
                ...
                DebugManager.LogMessage("GIF version error.\nSupported only GIF87a or GIF89a.");
                return false;
            }
		...
		private static bool SetGifHeader(NativeArray<byte> gifBytes, ref int byteIndex, ref GifData gifData) {
			// Signature(3 Bytes)
			// 0x47 0x49 0x46 (GIF)
			if (gifBytes[0] != 'G' || gifBytes[1] != 'I' || gifBytes[2] != 'F') {
				DebugManager.LogMessage("This is not GIF image.");
			...
			// Logical Screen Width(2 Bytes)
			gifData.m_logicalScreenWidth = BitConverter.ToUInt16(stackalloc byte[] { gifBytes[6], gifBytes[7] });
			//gifData.m_logicalScreenWidth = BitConverter.ToUInt16(gifBytes, 6);

			// Logical Screen Height(2 Bytes)
			gifData.m_logicalScreenHeight = BitConverter.ToUInt16(stackalloc byte[] { gifBytes[8], gifBytes[9] });
			//gifData.m_logicalScreenHeight = BitConverter.ToUInt16(gifBytes, 8);
            ...
            try {
                ...
                while (true) {
                    ...
                    if (lastIndex == nowIndex) {
						DebugManager.LogMessage("Infinite loop error.", DebugManager.ErrorLevel.Error);
                        return false;
                    }
		...
		private static bool SetGifBlock(NativeArray<byte> gifBytes, ref int byteIndex, ref GifData gifData) {
            try {
                int lastIndex = 0;
                while (true) {
                    int nowIndex = byteIndex;
        ...
        private static void SetImageBlock(NativeArray<byte> gifBytes, ref int byteIndex, ref GifData gifData) {
            ImageBlock ib = new ImageBlock();
            ...
            // Image Left Position(2 Bytes)
            ib.m_imageLeftPosition = BitConverter.ToUInt16(stackalloc byte[] { gifBytes[byteIndex++], gifBytes[byteIndex++] });
            //ib.m_imageLeftPosition = BitConverter.ToUInt16(gifBytes, byteIndex);
            //byteIndex += 2;

            // Image Top Position(2 Bytes)
            ib.m_imageTopPosition = BitConverter.ToUInt16(stackalloc byte[] { gifBytes[byteIndex++], gifBytes[byteIndex++] });
            //ib.m_imageTopPosition = BitConverter.ToUInt16(gifBytes, byteIndex);
            //byteIndex += 2;

            // Image Width(2 Bytes)
            ib.m_imageWidth = BitConverter.ToUInt16(stackalloc byte[] { gifBytes[byteIndex++], gifBytes[byteIndex++] });
            //ib.m_imageWidth = BitConverter.ToUInt16(gifBytes, byteIndex);
            //byteIndex += 2;

            // Image Height(2 Bytes)
            ib.m_imageHeight = BitConverter.ToUInt16(stackalloc byte[] { gifBytes[byteIndex++], gifBytes[byteIndex++] });
            //ib.m_imageHeight = BitConverter.ToUInt16(gifBytes, byteIndex);
            //byteIndex += 2;
        ...
        private static void SetGraphicControlExtension(NativeArray<byte> gifBytes, ref int byteIndex, ref GifData gifData) {
            GraphicControlExtension gcEx = new GraphicControlExtension();
            ...
            // Delay Time(2 Bytes)
            gcEx.m_delayTime = BitConverter.ToUInt16(stackalloc byte[] { gifBytes[byteIndex++], gifBytes[byteIndex++] });
            //BitConverter.ToUInt16(gifBytes, byteIndex);
            //byteIndex += 2;
        ...
        private static void SetCommentExtension(NativeArray<byte> gifBytes, ref int byteIndex, ref GifData gifData) {
            CommentExtension commentEx = new CommentExtension();
        ...
        private static void SetPlainTextExtension(NativeArray<byte> gifBytes, ref int byteIndex, ref GifData gifData) {
            PlainTextExtension plainTxtEx = new PlainTextExtension();
        ...
        private static void SetApplicationExtension(byte[] gifBytes, ref int byteIndex, ref GifData gifData) {
            ...
            while (true) {
				...
				ApplicationExtension.ApplicationDataBlock appDataBlock = new ApplicationExtension.ApplicationDataBlock();
        ...
        private static void SetApplicationExtension(NativeArray<byte> gifBytes, ref int byteIndex, ref GifData gifData) {
            // Extension Introducer(1 Byte)
            // 0x21
            gifData.m_appEx.m_extensionIntroducer = gifBytes[byteIndex++];
